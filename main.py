...
def bli(img,x_coord,y_coord,scale):
	...
	# adjust window, if it happens to be going out of the scope of the original image size
	if (tl_x < 0):
		br_x = br_x - tl_x
		tl_x = 0
	if (tl_y < 0):
	...
	width1=int(roi.shape[1])-1
	height1=int(roi.shape[0])-1
	...
	# The new image's array is initialised and perform bilinear interpolaration
	new_vec=[]
	for i in range(int(height2)):
		for j in range(int(width2)):
			x = int(width_ratio*j)
			y = int(height_ratio*i)
			x_diff = (width_ratio*j) - x
			y_diff = (height_ratio*i) - y
			# print (x,y,x_diff,y_diff)
			# check extreme co ordinate
			...
			if(newcol == 0):
				new_vec.append([])
			new_vec[newrow].append(newimg)
			count+=1

		output=np.uint8(new_vec)
	return output

if __name__=="__main__":
	path = '/home/nagesh/Desktop/Nokota_Horses_cropped.jpg'
	img=cv2.imread(path)

	# Pivot co-ordinates and scale are given
	x_coord = img.shape[0]/2
	y_coord = img.shape[1]/2
	scale_factor = float(1.5)


	output=bli(img,x_coord,y_coord,scale_factor)
	cv2.imshow('input_img',img)
	cv2.imshow('out_put_img',output)
	cv2.imwrite("bln_img.jpg",output)
